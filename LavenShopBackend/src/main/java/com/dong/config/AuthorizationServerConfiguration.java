    ...
    public SecurityFilterChain securityFilterChain(HttpSecurity http) throws Exception {
        ...
        return http
                ...
                .authorizeHttpRequests(authorizeRequests -> authorizeRequests
                        ...
                        .requestMatchers(HttpMethod.POST, "/api/oauth2/v1/token").permitAll()
...
//                        .requestMatchers(HttpMethod.POST, "/api/v1/order/paypal_capture/**").permitAll()
                        ...
                        .requestMatchers(HttpMethod.POST, "/api/v1/users/**").hasAuthority("SCOPE_ADMIN")
                        .requestMatchers(HttpMethod.PUT, "/api/v1/users/**").hasAuthority("SCOPE_ADMIN")
                        .requestMatchers(HttpMethod.DELETE, "/api/v1/users/**").hasAuthority("SCOPE_ADMIN")

                        // Phân quyền cho các thao tác với products chỉ dành cho admin
                        .requestMatchers(HttpMethod.POST, "/api/v1/products/**").hasAuthority("SCOPE_ADMIN")
                        .requestMatchers(HttpMethod.PUT, "/api/v1/products/**").hasAuthority("SCOPE_ADMIN")
                        .requestMatchers(HttpMethod.DELETE, "/api/v1/products/**").hasAuthority("SCOPE_ADMIN")

                        // Phân quyền cho các thao tác với categories chỉ dành cho admin
                        ...
                        // Phân quyền cho các thao tác với notifications chỉ dành cho admin
                        .requestMatchers(HttpMethod.POST, "/api/v1/notifications/**").hasAuthority("SCOPE_ADMIN")
                        .requestMatchers(HttpMethod.PUT, "/api/v1/notifications/**").hasAuthority("SCOPE_ADMIN")
                        .requestMatchers(HttpMethod.DELETE, "/api/v1/notifications/**").hasAuthority("SCOPE_ADMIN")

                        // Tất cả các yêu cầu khác phải được xác thực
                        .anyRequest().authenticated()
                )
                .formLogin(Customizer.withDefaults())  // Cấu hình login mặc định
                .oauth2ResourceServer(oauth2 -> oauth2
                        .jwt(Customizer.withDefaults())  // Sử dụng JWT cho OAuth2
                )
                .build();
    }
